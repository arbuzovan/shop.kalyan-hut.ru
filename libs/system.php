<?php
	...
	function system_checkSession() {
		return is_array($_COOKIE) && array_key_exists("umicms_session", $_COOKIE);
	}

	function system_setSession() {
		$sessionId = md5(time());
		setcookie("umicms_session", $sessionId, 0, "/");
		return $sessionId;
	}

	function system_removeSession() {
		setcookie("umicms_session", null, time() - 3600, "/");
	}

	function system_getSession() {
		return getCookie("umicms_session");
	}
