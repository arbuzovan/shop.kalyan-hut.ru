<?php
	...
	function check_session() {
		...
		if(is_null($session->get('session-owner-ip'))) {
			$session->set('session-owner-ip', $ip);
			return true;
		} elseif ($session->get('session-owner-ip') == $ip) {
			return true;
		} else {
			session::destroy();
			session::getInstance();
	...
	function enableOutputCompression() {
		if(extension_loaded('zlib') && !defined('DEBUG')) {
			...
			ob_start(array('umiExceptionHandler', 'outputBufferHandler'));
			echo $buffer;
		}
	}
	...
	function prepareRequest($key) {
		...
		if (!isset($_SESSION['paging'][$domainId][$langId][$module][$method]['per_page_limit'])) $_SESSION['paging'][$domainId][$langId][$module][$method]['per_page_limit'] = 20;
		...
		if ($key == 'per_page_limit') {
			...
			if (!is_null($perPageLimit)) {
				$oldLimit = $_SESSION['paging'][$domainId][$langId][$module][$method]['per_page_limit'];
				$_SESSION['paging'][$domainId][$langId][$module][$method]['per_page_limit'] = $perPageLimit;
				$pageRecalcCoef = $oldLimit/$perPageLimit;
				foreach ($_SESSION['paging'][$domainId][$langId][$module][$method] as $rel=>$page) {
					if ((string) $rel == 'per_page_limit') continue;
						$_SESSION['paging'][$domainId][$langId][$module][$method][$rel] = (int) ($page * $pageRecalcCoef);
				}
				if (($oldLimit != $perPageLimit) && isset($_REQUEST['p']) && $_REQUEST['p'] != 'all') {
					$_REQUEST['p'] = $_SESSION['paging'][$domainId][$langId][$module][$method][$relId];
				}
			}
			return $_SESSION['paging'][$domainId][$langId][$module][$method]['per_page_limit'];
		}
	...
	function elfinder_get_hash($path) {
		...
		if ($filemanagerDirectory = $user->getValue('filemanager_directory')) {
			...
			if (strpos($path, $images_path) === 0) {
				...
				$source = "images";
			} elseif (strpos($path, $files_path) === 0) {
		...
		return strlen($hash) ? "umi" . $source . "_" . $hash : "umi" . $source;
