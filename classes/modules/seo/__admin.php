<?php
	abstract class __seo extends baseModuleAdmin {
		...
		public function siteAnalyzeJson($login, $password, $site, $date) {
			...
			if ($json->status != 0) {
				throw new coreException(getLabel('error') . $json->error);
			}
		...
		public function getDatasetConfiguration($param = '') {
			switch($param) {
				case 'filter_links':
					$loadMethod = 'filter_links';
					$delMethod  = 'filter_links_delete';
					$typeId		= regedit::getInstance()->getVal("//modules/seo/filter_links_otype_id");
					$defaults	= 'url[500px]';
					$stoplist   = array('filters', 'domain_id');
				break;
				default:
					$loadMethod = 'islands';
