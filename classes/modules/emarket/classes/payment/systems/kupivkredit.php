<?php
	...
	class kupivkreditPayment extends payment {
		...
		public function __construct(iUmiObject $object, order $order = null) {
			...
			$this->api = new kvkAPI($this->getValue("apiKey"), $this->getValue("partnerId"), $this->getValue("secretKey"));
		}
		...
		public function validate() {
			return $this->order->getActualPrice() >= 3000;
		}
			...
			if(!getRequest('accepted')) {
				...
				if (!$response->isSuccess()) {
					...
					if(!$this->responseCheckItems($result)) {
						...
						if($module) {
							$module->redirect("{$controller->pre_lang}/emarket/purchase/result/{$step}/");
						}
