		...
		*/
		...
		public function updateYML() {
			...
			if ($this->is_active && !$this->is_deleted) {
				...
				foreach ($matches as $exportId) {
					file_put_contents($dirName . 'shop' . $exportId, '<name>' . iconv("UTF-8", "CP1251//IGNORE", $name) . '</name><company>' . iconv("UTF-8", "CP1251", $companyName) . '</company><url>https://' . domainsCollection::getInstance()->getDomain($this->getDomainId())->getHost() . '</url>');
				}
			}
		}
		...
		public function updateSiteMap($ignoreChilds = false) {
			...
			while(list($level) = mysql_fetch_row($result)) {
				...
				if($pagePriority <= 0.3) $pagePriority = $pagePriority + 0.5;
				if($pagePriority > 1) $pagePriority = 1;
			}
