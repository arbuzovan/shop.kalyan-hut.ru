		...
		*/
		public function delElement($element_id) {
			...
			if($element = $this->getElement($element_id)) {
				...
				$event->setParam("element_id", $element_id);
				$event->setMode("after");
				$event->call();
...
SQL;
		...
		*/
		public function getDeletedList(&$total = 0, $limit = 20, $page = 0) {
			$limit = intval($limit);
...
TOTALSQL;
			...
			$totalResult = l_mysql_query($totalSql);
			$totalRow = mysql_fetch_row($totalResult);
			if($totalRow) {
				$total = intval($totalRow[0]);
			}
		...
		*/
		...
		/**
			...
			* @param Boolean $ignoreUrlSuffix=false игнорировать суффикс ссылок.
			* @return String адрес страницы
		*/
		public function getPathById($element_id, $ignoreLang = false, $ignoreIsDefaultStatus = false, $ignoreCache = false, $ignoreUrlSuffix = false) {
			$element_id = (int) $element_id;
			...
			if($element = $hierarchy->getElement($element_id, true)) {
				...
				if(!$this->bForceAbsolutePath && $current_domain->getId() == $element_domain_id) {
					...
					$domain_str = "https://" . $domains->getDomain($element_domain_id)->getHost();
				}
			...
			if($parents = $this->getAllParents($element_id, false, $ignoreCache)) {
				...
				foreach($parents as $parentId) {
					...
					if (!$ignoreCache && isset($this->pathPiecesCache[$parentId])) continue;
				...
				if ($config->get('seo', 'url-suffix.add') && !$ignoreUrlSuffix) {
					$path .= $config->get('seo', 'url-suffix');
				}
		...
		*/
		public function getIdByPath($element_path, $show_disabled = false, &$errors_count = 0, $domain_id = false, $lang_id = false) {
			...
			if($urlSuffix = mainConfiguration::getInstance()->get('seo', 'url-suffix')){
				$pos = strrpos($element_path, $urlSuffix);
				if ($pos && ($pos + strlen($urlSuffix) == strlen($element_path))) {
					$element_path = substr($element_path, 0, $pos);
				}
			}
		...
		/**
			...
			* @param Integer $rel_id id родительской страницы
		...
		*/
		public function getDefaultElementId($lang_id = false, $domain_id = false) {
			...
			if (!$lang_id) $lang_id = $cmsController->getCurrentLang()->getId();
			if (!$domain_id) $domain_id = $cmsController->getCurrentDomain()->getId();
...
SQL;
		...
		*/
		...
		/**
			* Получить максимальный уровень вложенности для элемента
		...
		*/
		 ...
		 *
		 * @param array $arr массив, где ключ это id страницы, а значение равно false(!)
		 *
		...
		*/
		...
		//TODO Write here methods to rebuild cms3_hierarchy_relations subnodes
...
SQL;
		...
		*/
		 ...
		 * TODO PHPDoc
		 * Enter description here ...
		 * @param unknown_type $alt_name
		 * @param unknown_type $element
		 * @param unknown_type $b_fill_cavities
		 * @param unknown_type $ignore_cur_element
		 */
