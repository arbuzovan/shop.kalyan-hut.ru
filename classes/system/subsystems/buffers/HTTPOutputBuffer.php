<?php
	class HTTPOutputBuffer extends outputBuffer {
		...
		public function send() {
            $v7f2db423a49b305459147332fb01cf87 = $this->buffer;
            $v70e8822b2e035fa3777d666207aeafa8 = new umiEventPoint('systemBufferSend');
            $v70e8822b2e035fa3777d666207aeafa8->setMode('before');
            $v70e8822b2e035fa3777d666207aeafa8->addRef('buffer', $v7f2db423a49b305459147332fb01cf87);
            $v70e8822b2e035fa3777d666207aeafa8->call();
            $this->buffer = $v7f2db423a49b305459147332fb01cf87;
			$this->sendHeaders();
			echo $this->buffer;
			$this->clear();
		}
		...
		public function redirect($url, $textStatus = '301 Moved Permanently', $numStatus = 301) {
			...
			if (!isset($uriInfo['scheme']) && !isset($uriInfo['host'])) $url = "/" . ltrim($url, "/");
		...
		protected function checkHTTPAuth() {
			...
			if (!$login && !$password && $httpAuthorization = getRequest('umi_authorization')) {
			...
			if($login && $password) {
				...
				if($permissions->isAuth() == false) {
					if($user = $permissions->checkLogin($login, $password)) {
						$permissions->loginAsUser($user->getId());
					} else {
						$this->clear();
		...
		protected function sendDefaultHeaders() {
			...
			if (strpos(getServer('HTTP_USER_AGENT'), 'MSIE')) {
